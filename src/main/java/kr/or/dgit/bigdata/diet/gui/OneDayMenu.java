...
public class OneDayMenu {
	...
	private boolean makeOneDay(int goodCal, int oneDayCost) { // (권장 칼로리, 1일예산, 시도횟수)
		...
		while (true) {
			...
			// Menu newMenu = MenuService.getInstance().getMenu(x); 
			Menu newMenu = tempmenuList.get(x - 1); // 신menu = 랜덤menu
			...
			if (allCostSum < newCost && allCalSum < (goodCal + 100)) {
				...
				/*
				 * System.out.println("===================================");
				 * System.out.println(newCost); System.out.println("메뉴 개수 : "+
				 * n); System.out.println("1일 칼로리 합 : " + calSum);
				 * System.out.println("1일 총예산 합 : " + costSum);
				 * System.out.println("생선&해산물 개수 : " + seafoodCnt);
				 * System.out.println("고기 횟수 : " + meatCnt); System.out.println(
				 * "음료 칼로리 합 : " + drinkCalSum);
				 */
				break;
			}
		}
		...
		/***
		 * 본 문제는 예산을 지나치게 많이 책정되어서 최저예산(1일예산-1만원)보다 적은 금액으로도 쉽게 권장칼로리를 넘게 설계되어
		 * 있음
		 **/

		// 만약 1일 음료칼로리합이 500cal 넘거나 1일예산-만원 보다 적을 경우 다시 생성
		if ((drinkCalSum > 500) || ((oneDayCost - 10000) > costSum)) {
			if (((oneDayCost - 10000) > costSum))
				System.out.println("=== 실격 : 최소예산 미달로 탈락");
			if ((drinkCalSum > 500))
				System.out.println("=== 실격 : 1일 음료수 칼로리 초과로 탈락");
